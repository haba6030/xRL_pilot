...
def load_trajectories(h, data_dir='data/opponent_model_trajectories'):
...
def trajectories_to_state_action_pairs(trajectories):
    ...
    OPPONENT MODEL VERSION: trajectories is list of episodes,
    each episode is list of {'state': ..., 'action': ...} dicts

    Args:
        trajectories: List of episodes (each episode is list of step dicts)
    ...
    for episode in trajectories:
        # Each episode is a list of {'state': ..., 'action': ...} dicts
        for step in episode:
...
def main():
    ...
    available_h = []
    for h in [1, 2, 3, 4]:
        filepath = Path('data/opponent_model_trajectories') / f'trajectories_h{h}.pkl'
        if filepath.exists():
